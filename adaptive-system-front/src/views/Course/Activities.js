...
const Activities = () => {
  ...
  const activities =
    course.lessons &&
    course.lessons[lessonId].activities.filter(
      activity => !(activity?.weight < (user.yAxis + 1) * 0.5)
    )
  ...
  const handleGoToNextActivity = () => {
    if (activityId + 1 < activities.length) setActivityId(activityId + 1)
  }

  const handleGoToPrevActivity = () => {
    if (activityId - 1 > -1) setActivityId(activityId - 1)
  }
  ...
  const handleGoToLesson = () => {
    history.push(
      location.pathname.substring(
        0,
        location.pathname.lastIndexOf(lessonId) - 1
      )
    )
  }
