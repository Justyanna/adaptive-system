...
const getUsers = async(req, res, next) => {
    try {
        ...
        let users = await User.find({}, { _id: 1, courses: 1, roles: 1, email: 1, firstName: 1, lastName: 1, xAxis: 1, yAxis: 1 });
...
const checkQuestionnaire = async(req, res, next) => {
    try {
        ...
        if (user.questionnaire === null) return res.status(400).end('Questionnaire not available');
